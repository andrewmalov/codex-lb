...
The system SHALL allow the admin to create API keys via `POST /api/api-keys` with an optional `provider_scope` field. The field SHALL be a comma-separated subset of `{'codex', 'claude'}`. When `provider_scope` is omitted, the system SHALL default it to `'codex'`. Values outside the allowed set SHALL be rejected with HTTP 400.
...
The system SHALL allow updating `provider_scope` via `PATCH /api/api-keys/{id}`. The new value SHALL be validated as a comma-separated subset of `{'codex', 'claude'}`. Other key properties (`name`, `allowedModels`, `weeklyTokenLimit`, `expiresAt`, `isActive`) SHALL be unaffected.
...
`ApiKeyResponse` SHALL include `provider_scope` as a string.

#### Scenario: List keys returns provider_scope

- **WHEN** admin calls `GET /api/api-keys`
- **THEN** each key entry includes `providerScope`
