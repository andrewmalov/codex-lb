...
class LoadBalancer:
    ...
    async def _load_selection_inputs(
        ...
        *,
        ...
        cached = await self._selection_inputs_cache.get(cache_key)  # ty:ignore[invalid-argument-type]
            ...
            if model and not accounts:
                if not all_accounts:
                    ...
                    await self._selection_inputs_cache.set(
                        _clone_selection_inputs(selection_inputs), key=cache_key, generation=load_generation  # ty:ignore[invalid-argument-type]
                    )
                    return selection_inputs
